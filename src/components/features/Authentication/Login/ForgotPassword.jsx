  ...
  return (
    <section className='container mt-4'>
      <div className={`${styles.form} mx-auto`}>
        <h2 className='text-center border-bottom border-bottom-1 pb-4'>
          Forgot password
        </h2>
        <form onSubmit={handleSubmit}>
          {response?.status !== 'success' &&
            response?.data?.response?.data.message !==
              'Reset code is invalid or has expired' && (
              <div
                className={`input-group mb-3 ${styles.inputGroup} ${styles.forgotInputGroup}`}
              >
          ...
          {response && response?.data?.data?.status !== 'Success' && (
            <p
              className={`alert py-2 text-center ${response?.status === 'error' ? 'alert-danger' : 'alert-success'}`}
            >
          ...
          {((response?.status === 'success' &&
            response?.data?.data?.message === 'Reset code sent to your email') ||
            response?.data?.response?.data.message ===
              'Reset code is invalid or has expired') && (
            <div
              className={`input-group border-top pt-2 ${styles.inputGroup} ${styles.forgotInputGroup}`}
            >
              <label htmlFor='email' className='align-self-start mb-2'>
