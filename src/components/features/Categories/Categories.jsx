...
function Categories() {
  const { getCategories, getSubCategories, categories, subCategories, loading, error } =
        ...
        <div className='row row-cols-1 row-cols-md-2 row-cols-xl-3 g-2 p-2'>
          {categories?.map(category => {
            const hasSubCategories =
              subCategories?.filter(subCat => subCat.category === category._id).length > 0
            ...
            return (
              <div className='row g-3 m-0' key={category._id}>
                <div className='border rounded-2 overflow-hidden p-0 d-flex flex-column flex-md-row'>
                  <div
                    className={`${hasSubCategories ? 'col-12 col-md-6' : 'col-12'} mb-0`}
                  >
                    <Link
                  ...
                  {hasSubCategories ? (
                    <ul
                      className={`bg-white col-12 col-md-6 mb-0 ${styles.subCategories}`}
                    >
                      {subCategories
                        ?.filter(subCat => subCat.category === category._id)
                        ?.map(subCat => (
                          <li key={subCat._id}>
                            <Link to={`${subCat.slug}?type=subCat&id=${subCat._id}`}>
                              {subCat.name}
                            </Link>
                          </li>
                        ))}
